...
impl ToolsLoader {
    ...
    async fn tool_exists(&self, tool_id: &str) -> Result<bool> {
        ...
        log::debug!("Checking if tool '{}' exists", tool_id);
    ...
    async fn update_tool(&self, tool_id: &str, tool: &Value) -> Result<()> {
        ...
        log::debug!("PUT tool via {}", path);

        let response = self.client.put_json_value(&path, tool).await?;
...
mod tests {
    ...
    fn test_loader_creation() {
        ...
        let _loader = ToolsLoader::new(client);
    }
