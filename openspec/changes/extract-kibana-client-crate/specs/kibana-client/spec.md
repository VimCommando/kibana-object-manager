...
The `kibana-client` crate SHALL construct clients from explicit configuration values rather than CLI project directories.
...
The `kibana-client` crate SHALL expose reusable API modules for saved objects, spaces, agents, tools, and workflows.
...
The `kibana-client` crate SHALL support syncing all supported API families without requiring a `kibob` filesystem project.
...
The `kibana-client` crate SHALL expose a dedicated public error enum and crate-local `Result<T>` alias instead of exposing `eyre::Report` in public APIs.
...
- **WHEN** the `kibob` CLI calls `kibana-client`
...
The `kibana-client` crate SHALL use `tracing` for diagnostic instrumentation and SHALL NOT initialize global logging or tracing subscribers.
...
- **WHEN** a consumer uses `kibana-client`
