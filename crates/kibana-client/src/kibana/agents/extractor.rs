...
/// # Example
/// ```no_run
/// use kibana_client::kibana::agents::{AgentsExtractor, AgentsManifest, AgentEntry};
...
impl AgentsExtractor {
    ...
    pub async fn search_agents(&self, _query: Option<&str>) -> Result<Vec<Value>> {
        ...
        tracing::info!("Found {} agent(s) via search", agents.len());
    ...
    async fn fetch_manifest_agents(&self, manifest: &super::AgentsManifest) -> Result<Vec<Value>> {
        ...
        for entry in &manifest.agents {
            ...
            set.spawn(async move {
                ...
                tracing::debug!("Fetched agent: {}", agent_id);
                Ok::<Value, Error>(agent)
            });
        }
        ...
        tracing::info!("Fetched {} agent(s) from manifest", agents.len());
...
impl Extractor for AgentsExtractor {
    ...
    async fn extract(&self) -> Result<Vec<Self::Item>> {
        let agents = if let Some(manifest) = &self.manifest {
            ...
            tracing::warn!("No manifest provided - use search API to discover agents");
            Vec::new()
        };
