...
fn kibana_client_with_test_space(url: Url) -> Result<KibanaClient> {
...
async fn test_saved_objects_extract() -> Result<()> {
    ...
    let client = kibana_client_with_test_space(url)?;
...
async fn test_saved_objects_roundtrip() -> Result<()> {
    ...
    let manifest = SavedObjectsManifest::read("/tmp/kibana-test/test_manifest.json")?;
    let url = Url::parse("http://localhost:5601")?;
    let client = kibana_client_with_test_space(url.clone())?;
    let space_client = client.space("test")?;
    ...
    let verify_client = kibana_client_with_test_space(verify_url)?;
