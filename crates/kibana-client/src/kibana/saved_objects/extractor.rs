...
/// # Example
/// ```no_run
/// use kibana_client::kibana::saved_objects::{SavedObjectsExtractor, SavedObjectsManifest, SavedObject};
...
impl SavedObjectsExtractor {
    ...
    async fn export_objects(&self) -> Result<Vec<Value>> {
        ...
        tracing::info!("Exported {} object(s) from Kibana", objects.len());
...
impl Extractor for SavedObjectsExtractor {
    ...
    async fn extract(&self) -> Result<Vec<Self::Item>> {
        ...
        tracing::info!("Extracted {} saved object(s)", objects.len());
