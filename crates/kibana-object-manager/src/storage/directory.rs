    ...
    fn read_recursive(&self, dir: &Path, objects: &mut Vec<Value>) -> Result<()> {
        for entry in std::fs::read_dir(dir)? {
            ...
            if path.is_dir() {
                ...
                let value: Value = from_json5_str(&content).map_err(|err| {
                    Error::message(format!("Failed to parse JSON: {}: {err}", path.display()))
                })?;
...
impl DirectoryWriter {
    ...
    pub fn write_all(&self, items: &[Value]) -> Result<usize> {
        ...
        for item in items.iter() {
            ...
            let json =
                to_string_with_multiline(item).map_err(|err| Error::message(err.to_string()))?;
