...
**Migration**: `kibob` SHALL continue deciding when and where to load `spaces.yml`, then pass the resulting space registry into `kibana-sync` explicitly. Explicit filesystem bundle APIs in `kibana-sync` MAY read or write a spaces manifest when the caller provides the path.
