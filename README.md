...
Create a `manifest/workflows.yml` to also manage Kibana workflows:
```yaml
workflows:
  - id: workflow-123
    name: my-workflow
...
Fetch saved objects from Kibana (specified in manifest) and save to local files. Also pulls spaces if `manifest/spaces.yml` exists and workflows if `manifest/workflows.yml` exists.
- `--space <name>`: Override the Kibana space (overrides KIBANA_SPACE env var)

### `kibob push <dir> [--managed true|false] [--space <name>]`
Upload local objects to Kibana. Also pushes spaces if `manifest/spaces.yml` exists and workflows if `manifest/workflows.yml` exists.
...
- `bundle/workflows.ndjson` - Workflows (if manifest/workflows.yml exists)
...
│   └── workflows.yml         # (Optional) Tracks which workflows to manage
...
│   ├── marketing.json
│   └── engineering.json
├── workflows/                # (Optional) Workflow configurations
│   ├── my-workflow.json
│   ├── alert-workflow.json
│   └── data-pipeline.json
└── bundle/                   # (Generated by 'togo' command)
    ...
    └── workflows.ndjson      # Bundled workflows
```
