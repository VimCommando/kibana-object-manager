...
### Requirement: SpaceContext Module

**Reason**: Functionality merged into `KibanaClient`. Space iteration handled by CLI using `kibana.space_ids()`.

**Migration**: Replace `SpaceContext::load()` with `KibanaClient::try_new()`. Replace `space_context.target_space_ids()` with `kibana.space_ids()` or CLI-level filtering.
