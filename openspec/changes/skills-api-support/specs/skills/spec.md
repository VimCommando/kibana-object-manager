...
- **THEN** the system writes each Skill under `skills/{skill-directory}/`
...
- **AND** each referenced content object contains `relativePath` equal to the markdown file parent path relative to the Skill directory
...
- **AND** preserves original Skill `id`, referenced content `name`, and referenced content `relativePath` values when projecting back to Kibana JSON
...
- **AND** the request body includes `id`, `name`, `description`, `content`, `referenced_content`, `tool_ids`, and `experimental` when present
...
- **AND** the request body is projected from `SKILL.md` frontmatter, `SKILL.md` markdown body, and referenced markdown files
- **AND** the request body includes mutable fields such as `name`, `description`, `content`, `referenced_content`, `tool_ids`, and `experimental`
